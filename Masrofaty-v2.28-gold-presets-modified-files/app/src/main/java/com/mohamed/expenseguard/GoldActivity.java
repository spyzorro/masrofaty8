...
public class GoldActivity extends Activity {
    ...
    private void show() {
        ...
        page.addView(tv("🪙 مدخراتي من الذهب", 27, true));
        page.addView(tv("سجّل الوزن والعيار، والتطبيق يحسب قيمة البيع الحالية بالجنيه المصري.", 13, false));

        LinearLayout head = new LinearLayout(this);
        head.setOrientation(LinearLayout.VERTICAL);
        head.setPadding(dp(12), dp(10), dp(12), dp(10));
        head.setBackgroundColor(Color.WHITE);
        summary = tv("جاري الحساب...", 21, true);
        status = tv("", 12, false);
        ...
        update.setOnClickListener(v -> refreshPrices());
        ...
        page.addView(tv("اختيارات سريعة", 14, true));
    ...
    private void render() {
        ...
        for (ExpenseDbHelper.GoldHolding g : hs) {
            ...
            c.addView(tv(String.format(Locale.US, "%.3f جرام  •  القيمة الحالية %s", g.grams, money(value)), 14, false));
        ...
        summary.setText("القيمة الحالية: " + money(total) + "\nإجمالي الوزن: " + String.format(Locale.US, "%.3f جرام", grams) + (cost > 0 ? "\nالفرق عن الشراء: " + money(total - cost) : ""));
    ...
    private void refreshPrices() {
        ...
        new Thread(() -> {
            try {
                ...
                c.setRequestProperty("User-Agent", "Mozilla/5.0 Masrofaty/2.28");
                StringBuilder html = new StringBuilder();
                try (BufferedReader r = new BufferedReader(new InputStreamReader(c.getInputStream(), "UTF-8"))) {
                    String line;
                    while ((line = r.readLine()) != null) html.append(line).append(' ');
                }
                String text = html.toString().replaceAll("<[^>]+>", " ").replace("&nbsp;", " ");
                for (int i = 0; i < karats.length; i++) {
                    Pattern p = Pattern.compile("عيار\\s*" + karats[i] + ".*?بيع\\s*:?\\s*([0-9,.]+).*?شراء\\s*:?\\s*([0-9,.]+)", Pattern.DOTALL);
                    Matcher m = p.matcher(text);
                    if (!m.find()) throw new IOException("تعذر قراءة عيار " + karats[i]);
                ...
                runOnUiThread(() -> {
                    status.setText(priceLine() + "\nآخر تحديث: " + new SimpleDateFormat("dd/MM/yyyy hh:mm a", new Locale("ar")).format(new Date()) + " • المصدر: edahabapp.com • الحساب بسعر الشراء المنشور");
                    render();
                });
            } catch (Exception e) {
                runOnUiThread(() -> {
    ...
    private String priceLine() {
        ...
        for (int i = 0; i < karats.length; i++) {
            if (i > 0) s.append("  |  ");
            s.append(karats[i]).append(": ").append(String.format(Locale.US, "%.0f", buy[i]));
        }
        return "سعر شراء الجرام: " + s;
    }
}
